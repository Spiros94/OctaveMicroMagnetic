function set_n_demag(iteration, permut, func)
    ...
    for i = 1:n(1) % Nested loops to know the index
        for j = 1:n(2)
            for l = 1:n(3)
                value = 0.0;
                idx = zeros(1,3);
                idx(1) = mod( (i + n(1) -1),(2*n(1)) ) - n(1) + 0;  % WARNING -- REVIEW MOD CALC
                idx(2) = mod( (j + n(2) -1),(2*n(2)) ) - n(2) + 0;  % WARNING -- REVIEW MOD CALC  
                idx(3) = mod( (l + n(3) -1),(2*n(3)) ) - n(3) + 0;  % WARNING -- REVIEW MOD CALC
